...
`ci/spring-verification`'s real-client checks, and not the final UI (the
real console will eventually replace the page below, at the same
path — see the Roadmap for what that'll be built with).
...
16. The real console (`console-tyrian/`), built with [Tyrian](https://tyrian.indigoengine.io/) (Scala.js, Elm-style `Model`/`Msg`/`update`/`view`, effects behind `Cmd`/`Sub` using `cats.effect.IO`) rather than Angular or React — chosen specifically because the console's actual shape (event-history inspector, state viewer, request/response drill-down, a control surface for reset/clear/fault actions, a streaming-lifecycle visualizer) is closer to an explicit state machine than a general reactive website, and Tyrian's architecture makes every state transition a pure, browser-free-testable function. Full reasoning, the considered alternatives (Laminar as the stronger fallback if this hits real friction; React/Angular ruled out early), the planned `llmsim-management-api` cross-project module boundary, and the exact vertical-slice sequencing are all in `docs/console-framework-decision.md`.

    Served by the standalone image at `/_llmsim/ui`, with overview/calls/timeline/streaming views as designed. Deliberately last, and **no longer blocked** — item 14 (streaming fault injection) shipped in full in `v0.4.0`, so the shapes this console needs to be designed against (`StreamFault`, `CallOutcome.Cancelled`, `CapturedCall`) are now the real, stable contract, not a moving target. The bare-bones dashboard (item 13) can stay exactly as it is through the first vertical slice — there's no need to retire it before the replacement is real.
