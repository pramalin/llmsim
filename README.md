...
A bare-bones, read-only view of what the simulator's been doing, meant
for a human glancing at it during development — not a substitute for
`ci/spring-verification`'s real-client checks. Deliberately kept
alongside the real Tyrian console (`/_llmsim/console`, see the Roadmap)
rather than retired in its favor: this page has zero dependencies in
any deployment shape, including plain `sbt run` with nothing built,
where the console isn't necessarily present at all.

```bash
curl -s http://localhost:8089/_llmsim/dashboard
```
...
Every count here is scoped to the journal's *currently retained* calls,
not a lifetime total — `journal.retainedCalls`/`capacity` make that
scope visible in the payload itself, since the journal is bounded and
can be cleared independently of the script (see above). `average`/
`p95`/`max`/`lastCallAtEpochMillis` are `null`, not `0`, when the
journal is empty — a freshly booted, idle simulator reads as "no data
yet," not as a real (and wrong) latency figure. `p95` is nearest-rank
over the retained calls' durations.

`script.nextStepIndex` is `null` specifically when the script is
exhausted *and* its overrun policy is `"fail"` — under `"repeatLast"` or
`"cycle"` it's never null, since there's always a well-defined next
step. `script.name` is `null` unless llmsim was booted the normal way
(`sbt run` / the Docker images) — in-process tests that build an `App`
directly without going through `Main` don't have a class name to report.

`GET /_llmsim/dashboard` sends `Cache-Control: no-store` — this is meant
to be polled live, not cached by anything sitting between the page and
llmsim.

```bash
open http://localhost:8089/_llmsim/ui
```

`GET /_llmsim/ui` serves a single static HTML page — no build step, no
framework — that polls the JSON above every couple of seconds and
renders it. It shows a "last refreshed" timestamp and a visible error
...
src/main/scala/com/alai/llmsim/
  ...
  ManagementRoutes.scala -- test-harness routes: /_llmsim/calls, /status,
                             /reset, /dashboard, /ui
  Dashboard.scala        -- the dashboard's data model, aggregation, and
                             embedded HTML page (ManagementRoutes calls
                             into this; keeps routing and rendering separate)
...
src/test/scala/com/alai/llmsim/
  ...
  DashboardSpec.scala           -- Dashboard's pure aggregation function,
                                    ScriptRunner.status, and the two routes
...
docs/
  dashboard-design.md    -- design record for the dashboard: the JSON
                             contract, why ScriptStatus exists instead of
                             exposing ScriptRunner's raw position, and the
                             reasoning behind a couple of things that
                             changed after review. Not user documentation
                             (see "Dashboard" above for that) -- this is
                             the "why," kept separate from the "how."
```
...
16. ~~The real console (`console-tyrian/`)~~ — done, shipped in `v0.6.0`: built with [Tyrian](https://tyrian.indigoengine.io/) (Scala.js, Elm-style `Model`/`Msg`/`update`/`view`, effects behind `Cmd`/`Sub` using `cats.effect.IO`) rather than Angular or React — chosen specifically because the console's actual shape (event-history inspector, state viewer, request/response drill-down, a control surface for reset/clear actions, script status) is closer to an explicit state machine than a general reactive website, and Tyrian's architecture makes every state transition a pure, browser-free-testable function. Full reasoning, the considered alternatives (Laminar as the stronger fallback, never needed — Tyrian never hit the kind of friction that would have called for it; React/Angular ruled out early), the `common` cross-project module boundary, and the vertical-slice sequencing that was actually followed are all in `docs/console-framework-decision.md`.

    Served at `/_llmsim/console`, packaged into the release image itself (a dedicated Docker build stage builds it with Node/npm, entirely separate from the `llmsim-build` stage other projects extend — confirmed by real CI runs that Node never enters that published image, and that the documented `FROM llmsim-build` extension pattern is completely unaffected).

    **Deliberately not replacing `/_llmsim/ui`** — the two now have a permanent, not temporary, division of roles. The bare-bones dashboard (item 13) works with zero dependencies in any deployment shape, including plain `sbt run` with nothing built — no Node, no npm, no Docker required. The Tyrian console is richer (an actual call table, click-through detail view, live script status) but only exists once something has actually built and bundled it. Keeping both means llmsim always has a working dashboard regardless of how it's being run, with the fuller console available wherever it's present.
